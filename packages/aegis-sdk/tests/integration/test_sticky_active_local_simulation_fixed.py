...
class TestStickyActiveLocalSimulation:
    ...
    async def test_leader_election_with_three_instances(self, nats_adapter, logger):
        ...
        try:
            ...
            # Make RPC requests to all instances
            for service in services:
                result = await nats_adapter.rpc(
                    subject=f"sticky-test-service.{service.instance_id}.process_request",
                    payload={"request_id": "test-123"},
                    timeout=2.0,
                )

                if service.is_active:
                    # Active instance should process the request
                    assert result.success is True
                    assert result.result["instance_id"] == service.instance_id
                    logger.info(f"Active instance {service.instance_id} processed request")
                else:
                    # Standby instances should return NOT_ACTIVE error
                    assert result.success is False
                    assert result.error == "NOT_ACTIVE"
                    logger.info(f"Standby instance {service.instance_id} rejected request")
