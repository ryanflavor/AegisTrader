...
class ProjectGenerator:
    ...
    def __init__(self, template_generator: Any | None = None):
        """Initialize the project generator with template generators.

        Args:
            template_generator: Optional template generator implementation.
    ...
    def _call_generator_method(self, generator, method_name: str, config: BootstrapConfig) -> str:
        """Call a method from the template generator class or instance."""
        # Handle both class and instance
        if generator is None:
            # No generator available, use default
            return f'"""Generated by {method_name} for {config.project_name}."""\n'

        # Check if it's a class or instance
        generator_instance = generator() if isinstance(generator, type) else generator

        if hasattr(generator_instance, method_name):
            method = getattr(generator_instance, method_name)
            return method(config)
        else:
            # Fallback to default implementation if method doesn't exist
            return f'"""Generated by {method_name} for {config.project_name}."""\n'
    ...
    def _generate_application_structure(
        ...
        if config.template == ProjectTemplate.ENTERPRISE_DDD:
            # 企业级DDD架构
            # Use injected generator or fallback to None (will use defaults)
            generator = self._template_generator
            if True:  # Always generate structure even without generator
                # Domain layer - 核心业务逻辑
                files[f"{base_dir}/domain/__init__.py"] = self._generate_domain_init(config)
                files[f"{base_dir}/domain/entities.py"] = self._call_generator_method(
                    generator, "generate_domain_entities", config
                )
                files[f"{base_dir}/domain/value_objects.py"] = self._call_generator_method(
                    generator, "generate_domain_value_objects", config
                )
                files[f"{base_dir}/domain/repositories.py"] = self._call_generator_method(
                    generator, "generate_domain_repositories", config
                )
                files[f"{base_dir}/domain/services.py"] = self._generate_domain_services(config)

                # Application layer - 用例编排
                files[f"{base_dir}/application/__init__.py"] = self._generate_application_init(
                    config
                )
                files[f"{base_dir}/application/commands.py"] = self._call_generator_method(
                    generator, "generate_commands", config
                )
                files[f"{base_dir}/application/queries.py"] = self._call_generator_method(
                    generator, "generate_queries", config
                )
                files[f"{base_dir}/application/handlers.py"] = self._call_generator_method(
                    generator, "generate_handlers", config
                )

                # Infrastructure layer - 基础设施实现
                files[f"{base_dir}/infra/__init__.py"] = self._call_generator_method(
                    generator, "generate_infra_init", config
                )
                files[f"{base_dir}/infra/persistence.py"] = self._call_generator_method(
                    generator, "generate_persistence", config
                )
                files[f"{base_dir}/infra/messaging.py"] = self._call_generator_method(
                    generator, "generate_messaging", config
                )
                files[f"{base_dir}/infra/adapters.py"] = self._generate_adapters(config)

                # Crossdomain layer - 防腐层
                files[f"{base_dir}/crossdomain/__init__.py"] = self._call_generator_method(
                    generator, "generate_crossdomain_init", config
                )
                files[f"{base_dir}/crossdomain/translators.py"] = self._call_generator_method(
                    generator, "generate_translators", config
                )
                files[f"{base_dir}/crossdomain/anti_corruption.py"] = self._call_generator_method(
                    generator, "generate_anti_corruption", config
                )

                # Package layer - 纯工具函数
                files[f"{base_dir}/pkg/__init__.py"] = self._call_generator_method(
                    generator, "generate_pkg_init", config
                )
                files[f"{base_dir}/pkg/utils.py"] = self._call_generator_method(
                    generator, "generate_utils", config
                )
                files[f"{base_dir}/pkg/validators.py"] = self._call_generator_method(
                    generator, "generate_validators", config
                )

                # Types layer - 类型定义
                files[f"{base_dir}/types/__init__.py"] = self._call_generator_method(
                    generator, "generate_types_init", config
                )
                files[f"{base_dir}/types/dto.py"] = self._call_generator_method(
                    generator, "generate_dto", config
                )
                files[f"{base_dir}/types/interfaces.py"] = self._call_generator_method(
                    generator, "generate_interfaces", config
                )
        else:
            # 标准架构
            # Domain layer
    ...
    def analyze_test_results(
        ...
        if config.coverage:
            ...
            if not coverage_match:
                # Try simpler format: "Coverage: 85%"
